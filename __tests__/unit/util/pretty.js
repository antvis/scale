...
describe('pretty ticks', function() {
  it('pretty for c(0, 10)', () => {
    ...
    expect(res.ticks).to.be.eqls([ 0, 2, 4, 6, 8, 10 ]);
  });
  ...
  it('pretty for c(1, 11)', () => {
    ...
    expect(res.ticks).to.be.eqls([ 0, 2, 4, 6, 8, 10, 12 ]);
  });

  it('pretty for c(3, 97)', () => {
    const res = pretty(3, 97);
    expect(res.ticks).to.be.eqls([ 0, 20, 40, 60, 80, 100 ]);
  });

  it('pretty for c(-100, -10)', () => {
    const res = pretty(-100, -10);
    expect(res.ticks).to.be.eqls([ -100, -80, -60, -40, -20, 0 ]);
  });
  ...
  it('pretty for c(0, 0.0000267519)', () => {
    ...
    expect(res.ticks).to.be.eqls([ 0, 0.000005, 0.00001, 0.000015, 0.00002, 0.000025, 0.00003 ]);
  });

  it('pretty for c(0.0000237464, 0.0000586372)', () => {
    const res = pretty(0.0000237464, 0.0000586372);
    expect(res.ticks).to.be.eqls([
      0.00002,
      0.000025,
      0.00003,
      0.000035,
  ...
  it('pretty for c(0.243, 0.584, 0.987, 0.153, 0.433)', () => {
    ...
    expect(scale1.ticks).to.be.eqls([ 0, 0.2, 0.4, 0.6, 0.8, 1 ]);
